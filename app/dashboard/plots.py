...
def _add_day_night_bands(fig: go.Figure, ts: pd.Series, row: int = 1, col: int = 1) -> None:
    ...
    while day <= end:
        ...
        fig.add_vrect(
            x0=str(day),
            x1=str(min(night_end, end + pd.Timedelta(hours=1))),
...
def plot_energy_comparison(
    ...
    b = (
        _resample_hourly(baseline_df, ["timestamp", "total_electricity_kwh"])
        if not baseline_df.empty
        else pd.DataFrame()
    )
    o = (
        _resample_hourly(optimized_df, ["timestamp", "total_electricity_kwh"])
...
def plot_temperature_comparison(
    ...
    b = (
        _resample_hourly(baseline_df, ["timestamp", "indoor_temp_c"])
        if not baseline_df.empty
        else pd.DataFrame()
    )
    o = (
        _resample_hourly(
            optimized_df, ["timestamp", "indoor_temp_c", "cooling_setpoint_c", "heating_setpoint_c"]
        )
        if not optimized_df.empty
        else pd.DataFrame()
    )
    ...
    ts_ref = (
        b["timestamp"]
        if not b.empty
        else (o["timestamp"] if not o.empty else pd.Series(dtype="datetime64[ns]"))
    )
    if not ts_ref.empty:
...
def plot_hvac_power(
    ...
    b = (
        _resample_hourly(baseline_df, ["timestamp", "hvac_power_kw"])
        if not baseline_df.empty
        else pd.DataFrame()
    )
    o = (
        _resample_hourly(optimized_df, ["timestamp", "hvac_power_kw"])
        if not optimized_df.empty
        else pd.DataFrame()
    )
...
def plot_carbon_emissions(
    ...
    b = (
        _resample_hourly(baseline_df, ["timestamp", "carbon_kg_co2"])
        if not baseline_df.empty
        else pd.DataFrame()
    )
    o = (
        _resample_hourly(optimized_df, ["timestamp", "carbon_kg_co2"])
        if not optimized_df.empty
        else pd.DataFrame()
    )
...
def plot_energy_breakdown(df: pd.DataFrame, mode: str = "optimized") -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            x=h["timestamp"],
            y=h["hvac_electricity_kwh"],
            stackgroup="one",
            name="HVAC",
            line=dict(width=0),
            fillcolor="rgba(99,110,250,0.75)",
    ...
    fig.add_trace(
        go.Scatter(
            x=h["timestamp"],
            y=h["lighting_electricity_kwh"],
            stackgroup="one",
            name="Lighting",
            line=dict(width=0),
            fillcolor="rgba(0,204,150,0.75)",
    ...
    fig.add_trace(
        go.Scatter(
            x=h["timestamp"],
            y=h["equipment_electricity_kwh"],
            stackgroup="one",
            name="Equipment",
            line=dict(width=0),
            fillcolor="rgba(171,99,250,0.75)",
